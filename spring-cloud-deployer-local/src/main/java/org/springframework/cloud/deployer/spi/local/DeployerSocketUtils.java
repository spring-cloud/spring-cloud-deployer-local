 ...
 * @author Glenn Renfro
 */
public abstract class DeployerSocketUtils {
	...
	public static SortedSet<Integer> findAvailableTcpPorts(int numRequested, int minPort, int maxPort) {
		return findAvailablePorts(numRequested, minPort, maxPort);
	}
	 ...
	 *                               not be found
	 */
	private static SortedSet<Integer> findAvailablePorts(int numRequested, int minPort, int maxPort) {
		Assert.isTrue(minPort > 0, "'minPort' must be greater than 0");
		...
		if (availablePorts.size() != numRequested) {
			throw new IllegalStateException(
					String.format("Could not find %d available TCP ports in the range [%d, %d]",
